...
// Accepts both the raw CSV column names and common PascalCase/camelCase variants
// that Power BI might apply after data transformation.
...
// target_cluster is optional — used for hover text if present
...
export class Visual implements IVisual {
    ...
    public update(options: VisualUpdateOptions): void {
        ...
        const missing = Object.keys(COL_ALIASES)
            .filter(k => ci[k] === -1);
        ...
        // ── unique node table ─────────────────────────────────────────────────
        // Source rows carry definitive coordinates and cluster assignments.
        // A target-only node falls back to target_x/y/z and tgtCluster (if present).
        ...
        // ── edge traces (one per edge, opacity/width varies by weight) ────────
    ...
    private showMessage(text: string, color: string): void {
        ...
        // Use DOM methods — innerHTML is blocked by powerbi-visuals/no-html-injection ESLint rule
